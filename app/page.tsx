...
export default async function HomePage() {
  const [featured, latest, species, banners, reviews, avgRating] = await Promise.all([
    getFeaturedBirds(),
    getLatestBirds(8),
    getAllSpecies(),
    getActiveBanners(),
      ...
      {featured.length > 0 && (
        <section className="px-4 py-8 max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/birds?featured=true" className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:underline">
              Lihat semua →
            </Link>
          </div>
      ...
      {latest.length > 0 && (
        <section className="px-4 py-8 max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/birds" className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:underline">
              Lihat semua →
            </Link>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
